...
You can pass variables to the client-side using a `<GTProvider>`, or import from `gt-react/client` to use them in your default dictionary.
...
```js copy
import { T, Num } from "@i18n" // make sure you are exporting <Num> alongside <T>
...
export default function Page() {
    ...
    return (
        <main>
            <T id='messages'>
                You have <Num>{n}</Num> new messages.
            </T>
        </main>
    )
}
...
```js copy
import { Plural, Num } from "@i18n" // make sure you are exporting these
...
Learn more about plurals [here](/server/plural), how to use plurals with dictionaries [here](/dictionaries/server-t#plural-t), or how to use plurals on the client-side [here](/client/client-plural).
...
- Read more about translating [Client Components](/client) with `<GTProvider>`.

- Read more about [variables](/server/variables) like `<Var>`, `<Num>`, `<Currency>`, and `<DateTime>`.

- Read more about translating plural forms with [`<Plural>`](/server/plural).
