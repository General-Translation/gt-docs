...
export const { t, getGT, GTProvider } = initGT({
...
To get a translation in a server component, use the `getGT()` function to create `t()`. Then pass `t()` the appropriate `id` string.
