...
export const { t, GTProvider } = initGT({
    ...
    approvedLocales: ['en-US', 'fr', 'de', 'es', 'it', 'ja'] // the languages you want your app to be available in
});
```
...
export const { t, GTProvider } = initGT({
    defaultLocale: 'en-US'
    approvedLocales: ['en-US', 'fr', 'de', 'es', 'it', 'ja'],
    dictionary
});
```
...
To get a translation in a server component, use the `t()` function and pass the appropriate `id` string.
...
You can also import the `t()` component directly, **but only in server components**:

<Callout type="info">
    If you're not sure whether a component will be rendered on the client or the server, you should use `getGT()` to safely create the `t()` function.
</Callout>
