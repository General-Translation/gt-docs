...
You should add your `defaultLocale` and `locales` from [`next.config.js`](/gt-next/config). The reason you have to add these separately is that on certain hosts (e.g. Vercel), middleware runs on the edge, meaning `createNextMiddleware()` does not have access to your `initGT()` configuration.
