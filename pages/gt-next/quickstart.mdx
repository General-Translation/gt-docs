...
If you only want to provide a subset of translations to the client, use the `id` prop of `<GTProvider>`.
All client-side `<T>` components should have `id` props prefixed with this `id`.
