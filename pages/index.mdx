...
export const { I18N, GTProvider } = createGT({
    defaultLocale: 'en-US' // the language of your app
});
```

`i18n.js` is a central config file for `gt-react` components. `<I18N>` and other server-side `gt-react` functions are exported from it, not from the library directly.
...
`<I18N>` is a server-only component that surrounds any text or tags you'd like to be internationalized. 
Best practice is to pass a unique id prop to each `<I18N>` component. 
...
- A version of your page without text is served to your user, so that the text appears to be loading. When the translated content is ready, usually within a few seconds, it is rendered on the page.

- This is known as the "skeleton" render method, but `gt-react` can also be [configured](/gt-react/config) to handle the initial translation delay differently. Other options include serving English and then replacing it when the translation is ready, hanging the page while the translation loads, or rendering the translation only on the second page access.
