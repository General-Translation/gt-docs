...
export const { T, Plural, Value, GTProvider, intl, Var, Num, Currency, DateTime } = createGT({
    defaultLocale: 'en-US'
})

/* SERVER-SIDE I18N */

// To protect your API keys, translation never happens on the client. 
// These functions are specifically intended for use in Server Components and in server-side async functions. 
...
// `options` (object) - An object containing an optional `userLanguage`, in addition to non-essential parameters like `id`. See full list of `intl()` options at docs.generaltranslation.com/server/intl#options.
...
// `id` (string, best practice) - An ID string to avoid conflicts in the unified i18n sheet. See full list of `<GTProvider>` props at docs.generaltranslation.com/client/gtprovider#props.
...
// `name` (string, optional, defaults to "value") - Used for identification while rendering `<Value>` or `<Plural>` components.
...
// The `<Plural>` component is a special type of `<Value>` that handles and generates plural forms automatically, language-by-language. It works out of the box on forms that don't even exist in English, like the Arabic dual.
...
// To use `<Plural>` on the server-side:
...
// Warning: `<Plural>` cannot generate additional forms when it is used on the client side. It can only use the forms it is provided.
// To use `<Plural>` and `<Num>` on the client-side:
...
// ClientComponent.js
'use client'
import { useGT, Plural } from 'gt-react/client'
import { useState } from 'react'
...
// `name` (string, optional, defaults to "n") - Used for identification while rendering `<Value>` or `<Plural>` components.
...
// Tip: write variables as self-closing, like <Var/> and they will use their default name. In this way, `<Num>` can be used natively with `<Plural>`, without providing a name prop, and `<Var>` can be used natively with `<Value>`.
...
import { useLocale, useDefaultLocale } from 'gt-react/client'
...
const { Var } = createGT({ defaultLocale: 'en-US' });
export const { t, dict, GTProvider } = createGT({
    defaultLocale: 'en-US',
    dictionary: {
...
// The translations should be available to the `t()`, `<T>`, `<Value>`, `<Plural>`, `intl()`, and `<GTProvider>` functions.
