...
```js
import { initGT } from 'gt-react/next'; // or 'gt-react' in non-Next.js frameworks
...
const { Var, Num, Currency, DateTime } = initVariables({
    defaultLocale: 'en-US'
});
...
Sometimes it is difficult to pass children directly from server to client, for example if the client component is displayed conditionally, or is nested within another client component. 
...
These translations can then be accessed with the client-side `getGT()` hook. (`getGT()` will work on both the client and server.)

```js
import { GTProvider } from '@i18n' // you should only use this in server components
