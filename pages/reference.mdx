...
<Callout>
    Chat to the GPT-powered [setup wizard](https://chatgpt.com/g/g-NRR9teLEv-gt-react-wizard).
</Callout>
...
// renderMethod (string, optional) - How translations are rendered for the first time. Options are "replace", "hang", "subtle". Default is "skeleton".
// renderTimeout (string, optional) - Timeout before rendering a new translation is called off. Default is 8.5 seconds.
...
export const { I18N, intl, GTProvider, Numeric, Value, Variable, NumberVariable, CurrencyVariable, DateVariable } = createGT({
...
/* <I18N> */
// The `<I18N>` component is a server-side component which translates its children into the user's current language. How this translation is first displayed depends on the current render method.
...
// Any children or default values of `<Variable>` remain private, and are never sent to the General Translation cloud service.

// To use it on the server:

import { Variable } from '@i18n'
...
// On the server, it is usually used inside an `<I18N>` component.
// On the client, it's child is usually the result of a `t()` function.
...
// To use `<Value>` inside `<I18N>` on the server:

import { I18N, Value } from '@i18n'
...
// Note that `<Value>` is providing the value for the `<Variable name="name" />` component. This means that branches can be defined in another file and imported.
...
// To use `<Value>` with `<Variable>` on the client:

// page.js
import { GTProvider, Variable } from '@i18n' 
import ClientNameComponent from './ClientComponent.js'

export default function Page() {
    return (
        <GTProvider dictionary={{
            "greeting": <>Your name is <Variable>{name}</Variable>.</>
        }}>
            <ClientNameComponent />
        </GTProvider>
    )
...
import { I18N, Numeric, NumberVariable } from "@i18n"
// where `<NumberVariable>` is a special type of `<Variable>` which formats a number for a given locale
...
// To use `<Numeric>` and `<NumberVariable>` on the client-side:

// page.js
import { GTProvider, NumberVariable } from '@i18n' 
import ClientComponent from './ClientComponent.js'
...
import { Variable } from '@i18n' // on the server  server
...
import { NumberVariable } from '@i18n' // server
...
import { DateVariable } from '@i18n' // server
...
import { CurrencyVariable } from '@i18n' // server

// On the client, must be used as the descendant of a `<GTProvider>`:

import { CurrencyVariable } from 'gt-react/client'

// Tip: write variables as self-closing, like <Variable/> and they will use their default name. In this way, `<NumberVariable>` can be used natively with `<Numeric>`, without providing a name prop, and `<Variable>` can be used natively with `<Value>`.
...
// Use the `createVariables()` function from `gt-react` or `gt-react/next`:

// i18n.js
import { createGT, createVariables } from 'gt-react/next'
const { Variable } = createVariables({ defaultLanguage: 'en-US' });
export const { t, dict, GTProvider } = createGT({
    defaultLanguage: 'en-US',
    dictionary: {
        "variable.component": <>Your name is <Variable name="name" /></>
    }
})

/* How to provide your own translations, rather than using automatic translation? */

// Local translation dictionaries must use the following format:

{
    [id]: { k: key, t: translation },
    ...
}

// where `id` is a string, `key` is the SHA-256 hash (if a component) or the string in its default language (if a string), and `translation` is the translated content
// It is best to use the General Translation platform at generaltranslation.com to create these.
...
// The translations should be available to the `t()`, `<I18N>`, `intl()`, and `<GTProvider>` functions.
```