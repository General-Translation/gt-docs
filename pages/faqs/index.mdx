...
- This is known as the "replace" render method, but `gt-react` can also be [configured](/config) to handle the initial translation delay differently. Other options include serving web pages with the text still loading, hanging the page while the translation loads, or rendering the translation only on the second page access.
