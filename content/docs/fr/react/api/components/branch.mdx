 ...
 * Combinez `<Branch>` avec d'autres composants, tels que `<T>` pour les traductions et [composants variables](/docs/react/reference/using-branching-components) pour le contenu dynamique, afin de créer des interfaces riches et localisées.

## Prochaines étapes
 * Pour une utilisation plus avancée et des exemples, consultez [Utilisation des composants de branchement](/docs/react/reference/using-branching-components).
 * Pour en savoir plus sur les composants variables comme `<Currency>`, `<DateTime>`, `<Num>`, et `<Var>`, consultez la documentation [Utilisation des composants variables](/docs/react/reference/using-variable-components).