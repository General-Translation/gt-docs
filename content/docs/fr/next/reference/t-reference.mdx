---
title: Référence <T>
description: Ceci est une exploration détaillée du composant <T> et de la bibliothèque gt-next.
---


## Aperçu

Cet article va plonger en profondeur dans l'utilisation du [composant `<T>`](/docs/next/api/components/t) dans la bibliothèque `gt-next`.
...
Si vous cherchez comment utiliser les [Composants Variables](/docs/next/reference/using-variable-components) ou les [Composants de Bifurcation](/docs/next/reference/using-branching-components), veuillez vous référer à leurs articles respectifs.

---

## Qu'est-ce que le composant `<T>` ?

Le composant `<T>` est le moyen principal de traduire du texte et du contenu dans la bibliothèque `gt-next`.
Il permet la traduction en ligne de texte et de structures JSX, offrant une manière plus directe de gérer les traductions dans votre application.
...
Pour les composants côté client, `<T>` **DOIT** être enveloppé dans un [`<GTProvider>`](/docs/next/api/components/gtprovider) à un niveau supérieur.
...
#### Exceptions : Composants `<T>` côté serveur [#server-t]

Pour le rendu côté serveur, le `<GTProvider>` est optionnel pour les composants `<T>`.
...
export default function RootLayout({ children }) {
  ...
  return (
    <html lang={locale}>
      <body>
        <T> Bonjour, monde ! </T>  // [!code highlight]
...
Pensez à des éléments comme le nom d'un utilisateur : c'est (1) différent d'une personne à l'autre et (2) ne doit pas changer même lorsque la langue change.
...
Il existe de nombreux composants de variables : [`<Var>`](/docs/next/api/components/var), [`<Num>`](/docs/next/api/components/num), [`<Currency>`](/docs/next/api/components/currency) et [`<DateTime>`](/docs/next/api/components/datetime).
Certains de ces composants reformateront automatiquement le contenu en fonction de la locale de l'utilisateur. D'autres marqueront simplement le contenu comme "ne pas traduire".
De plus, ces composants n'ont pas nécessairement besoin d'être enveloppés dans un composant `<T>` pour reformater le contenu.

Lisez-en plus à ce sujet dans [cet article sur les composants de variables](/docs/next/reference/using-variable-components).

### Ramification dans les Composants `<T>`

Les composants de ramification peuvent être utilisés pour afficher un contenu différent en fonction des conditions pour les traductions.
Cela inclut les composants [`<Plural>`](/docs/next/api/components/plural) et [`<Branch>`](/docs/next/api/components/branch).
Nous utilisons ces composants car ils permettent un processus de traduction plus optimisé.

Par exemple, le composant `<Plural>` vous permet de marquer des phrases qui doivent être pluralisées lorsque la traduction a lieu.
...
Il est également utile car il standardise votre logique de pluralisation à travers votre application dans votre langue de base.

Vous pouvez spécifier différentes options selon le composant que vous utilisez.
Lisez-en plus à ce sujet dans l'article [Composants de Ramification](/docs/next/reference/using-branching-components).

### Traduction de Chaînes [#translating-strings]

Si vous souhaitez traduire simplement une chaîne, vous pouvez utiliser [`useGT()`](/docs/next/api/components/strings/useGT), [`getGT()`](/docs/next/api/components/strings/getGT), ou [`tx()`](/docs/next/api/components/strings/tx).

[`useGT()`](/docs/next/api/components/strings/useGT) et [`getGT()`](/docs/next/api/components/strings/getGT) sont utilisés pour traduire des chaînes au moment de la construction.
...
[`tx()`](/docs/next/api/components/strings/tx) est utilisé pour traduire des chaînes de caractères à l'exécution.
...
Pour plus d'informations, vous pouvez vous référer à [notre guide sur la traduction des chaînes](/docs/next/reference/translating-strings).
...
Cela vous permet également d'utiliser le [Translation Editor](/signin) pour apporter des modifications à vos traductions sans redéployer.
...
Pour un guide plus détaillé sur le déploiement de votre application, veuillez vous référer au guide de [Déploiement](/docs/next/tutorials/quickdeploy).
...
Cela pourrait ne pas être souhaitable pour le rendu de données sensibles, telles que les noms d'utilisateur, les numéros de compte, etc.

Pour contourner ce problème, utilisez les [Composants Variables](/docs/next/reference/using-variable-components#privacy) pour gérer les informations privées.
...
Votre premier réflexe pourrait être d'ajouter un composant `<T>` supplémentaire à l'intérieur de `<UntranslatedContent>`, mais cela n'est pas recommandé.
...
Nous recommandons d'utiliser les [Composants de Variable](/docs/next/reference/using-variable-components) pour afficher des valeurs dynamiques et les [Composants de Bifurcation](/docs/next/reference/using-branching-components) pour la logique conditionnelle.

Si vous souhaitez traduire des contenus dynamiques à l'exécution, consultez [`tx()`](/docs/next/api/components/strings/tx) et [`<Tx>`](/docs/next/api/components/tx).
...
La solution ici est de supprimer le composant `<T>` le plus externe.
...
Il est toujours préférable d'envelopper le contenu conditionnel dans un [Composant de Bifurcation](/docs/next/reference/using-branching-components) pour optimiser la traduction du contenu.
 ...
 * Enveloppez votre application avec un [`<GTProvider>`](/docs/next/api/components/gtprovider) pour activer les traductions dans les composants client.
 * `<T>` traduit les contenus au moment de la construction, pas à l'exécution. Le développement effectuera la traduction à la demande pour plus de commodité, mais ce n'est pas le cas en production.

## Prochaines étapes
 * Explorez l'[article sur les composants variables](/docs/next/reference/using-variable-components) et le [guide des composants de branchement](/docs/next/references/using-branching-components).
 * Apprenez à propos de [la traduction des chaînes](/docs/next/reference/translating-strings) dans votre application.
 * Pour la traduction en temps réel, consultez [`tx()`](/docs/next/api/components/strings/tx) et [`<Tx>`](/docs/next/api/components/tx).
