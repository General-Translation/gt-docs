...
<Callout>
  Utilisez-vous le routeur de pages Next.js ? Suivez plutôt le guide [Démarrage rapide React](/docs/react/tutorials/quickstart).
</Callout>
...
Vous pouvez spécifier les langues que vous souhaitez prendre en charge en passant un tableau de [codes de langue](/docs/reference/supported-locales).
...
Enveloppez tout contenu JSX imbriqué dans le [composant `<T>`](/docs/next/api/components/t) pour le rendre traduisible.
Pour plus d'informations, consultez le [guide d'utilisation des composants `<T>`](/docs/next/reference/t-reference).
...
<Callout type="info">
  **Astuce :**
  ...
  Cela analysera votre code pour trouver le JSX traduisible et insérera les balises [`<T>`](/docs/next/api/components/t) pour vous.
...
<Steps>
  <Step>
    ...
    Allez à la page **Developer Keys** dans la barre latérale.
  </Step>
...
<Callout type="warn">
  ...
  *Ne publiez jamais vos clés API dans un dépôt public !*
</Callout>
...
### Voir votre application dans une langue différente [#see-your-app-in-a-different-language]
...
<Accordions>
  ...
  <Accordion title="Pourquoi les langues prennent-elles beaucoup de temps à charger en développement ?">
  ...
  <Accordion title="Pourquoi certaines choses sont-elles traduites et d'autres non ?">
    **Traduction JSX**
    ...
    En attendant, si vous affichez une chaîne, veuillez suivre [ce guide](/docs/next/reference/translating-strings) pour traduire les chaînes.
  </Accordion>
</Accordions>
