---
title: <Plural>
description: Référence API pour le composant <Plural>
---
...
Lisez plus sur les différentes formes [ici](https://cldr.unicode.org/index/cldr-spec/plural-rules).
...
## Prochaines étapes
 * Pour plus d'exemples, consultez le document de référence sur les composants de branchement [ici](/docs/next/reference/using-branching-components).
 * Pour une utilisation plus avancée, combinez `<Plural>` avec des composants variables comme `<Currency>`, `<DateTime>`, `<Num>`, et `<Var>`. Lisez plus sur les composants variables [ici](/docs/next/reference/using-variable-components).

Pour approfondir la pluralisation et le branchement, visitez [Utilisation des composants de branchement](/docs/next/reference/using-branching-components).