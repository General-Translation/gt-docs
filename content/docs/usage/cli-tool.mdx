...
```bash copy
npx translate --locales fr es ja
```
...
```bash copy
npx translate --api-key YOUR_API_KEY --project-id YOUR_PROJECT_ID --locales fr es ja
```
...
To translate your app in advance, rerun with the --locales flag. For example: `npx translate --locales fr`
```
