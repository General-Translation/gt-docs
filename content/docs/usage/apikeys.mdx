 ...
 * The [`gtx-cli`](/docs/cli/introduction) tool when translations are generated during the deployment process. Check out [this guide](/docs/react/tutorials/quickdeploy) for more information.
 * The `gt-next` library when translations occur at runtime (server-side only, not used by `gt-react`).
