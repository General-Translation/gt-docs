...
An API key is a 36-character string beginning `gtx-` which is used to authenticate CLI tool with our cloud translation service. Create one on the [API Keys](https://generaltranslation.com/dashboard/redirect/api-keys) page.
...
<Tabs items={["npm", "yarn", "bun", "pnpm"]}>
  <Tab value="npm">```bash copy npm i gt-react ```</Tab>
  <Tab value="yarn">```bash copy yarn add gt-react ```</Tab>
  <Tab value="bun">```bash copy bun add gt-react ```</Tab>
  <Tab value="pnpm">```bash copy pnpm add gt-react ```</Tab>
</Tabs>
...
```bash copy
npx gt-react-cli translate --languages fr es ja
```
...
```bash copy
npx gt-react-cli translate --apiKey YOUR_API_KEY --projectID YOUR_PROJECT_ID --languages fr es ja
```
