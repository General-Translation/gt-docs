...
Let's say that you have added the [`gt-react-cli translate`](/docs/react-cli/translate) command to your CD process.
...
<Callout>
  **Note:**
  If you are not using GT infrastructure, you will need to write a custom [`loadTranslation()`](/docs/react/api/config/load-translation) method to load translations from your preferred infrastructure.
</Callout>
...
<Steps>
  ...
  <Step>
    ...
    export default function App() {
      return (
        <GTProvider loadTranslation={loadTranslation} locales={['es', 'fr']}> // [!code highlight]
          <YourApp />
        </GTProvider>
      );
    }
  ...
  <Step>
    In your [translate](/docs/react-cli/translate) command, add the following flags to use local translations:
     * `--no-publish`: Disable translations from being published to the GT CDN.
     * `--translations-dir ./src/_gt`: Save translations to the `./src/_gt` directory.
 ...
 * You can customize [`loadTranslation()`](/docs/react/api/config/load-translation) to also load translations from other sources, such as your database or your own CDN.

## Next steps
 * See [`loadTranslation()`](/docs/react/api/config/load-translation) for more information on writing a custom translation loader.
