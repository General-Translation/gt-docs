...
- The `<GTProvider>` component is used for both [inline `<T>`](/docs/react/reference/t-reference) and [dictionaries](/docs/react/reference/dictionary).
...
For more information on using dictionaries, check out this [guide](/docs/react/reference/dictionary).
...
It also directly integrates with the [`gt-react-cli translate` command](/docs/react-cli/translate), so you don't have to specify things like locales manually as a parameter.
...
You can use the `loadTranslation` prop to load translations from a custom source.
...
createRoot(document.getElementById("root")!).render(
  <StrictMode>
    <GTProvider locales={['es', 'fr']} loadTranslation={loadTranslation}> // [!code highlight]
      <App />
    </GTProvider>
  </StrictMode>
);
...
## Notes
 * The `<GTProvider>` must wrap all [`<T>` components](/docs/react/api/components/t) and other translation-related functions. Read more about it [here](/docs/react/reference/t-reference#gtprovider).
