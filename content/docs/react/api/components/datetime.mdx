...
const dictionary = {
    "eventTime": <>
        The event is at <DateTime>{1738010355}</DateTime>. // [!code highlight]
    </>,
};
export default dictionary;
```

```jsx title="EventTime.jsx" copy
import { useGT } from 'gt-react';

export default function EventTime() {
    const t = useGT();
    return <> t('eventTime') </>; // [!code highlight]
}
```
...
import { DateTime } from 'gt-react';
const dictionary = {
    "eventTime": <>The event is at <DateTime name='time'/>.</>, // [!code highlight]
};
export default dictionary;
```
...
export default function EventTime() {
    const t = useGT();
    return <> t('eventTime', { time: 1738010355 }) </>; // [!code highlight]
}
```
