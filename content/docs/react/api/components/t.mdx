...
<Callout>
    For production, to ensure translations are available, run the [CLI tool](/docs/react/reference/cli-tool) before deploying your app.
</Callout>
...
2. **Privacy**: *The children of `<Currency>`, `<DateTime>`, `<Num>`, or `<Var>` are NEVER sent to the translation API.*
  Because the children of the `<T>` component are sent to the translation API, if you have sensitive information within a `<T>` tag, wrap it in one of the [variable tags](/docs/react/reference/using-variable-components).
...
You can [read more about this topic here](/docs/react/reference/t-reference#direct-text).
 ...
 * For translating strings, look into [`tx()`](/docs/react/api/strings/tx) or [`getGT()`](/docs/react/api/dictionary/getGT) for server-side translations and [`useGT()`](/docs/react/api/dictionary/useGT) for client-side translations.
