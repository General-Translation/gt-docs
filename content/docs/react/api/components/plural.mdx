...
We have also used the `<Num>` component and given it the same name as the `<Plural>` component.
This way the same value will be passed to both the `<Plural>` and `<Num>` components.
```jsx title="dictionary.jsx" copy
import { Plural } from 'gt-react';

const dictionary = {
  itemCount: <>
    <Plural name='count' // [!code highlight]
      singular={You have <Num name='count'/> item.} // [!code highlight]
      plural={You have <Num name='count'/> items.} // [!code highlight]
    />
  </>
}
```
...
export default function ItemCount({ count }) {
  const t = useGT();
  return (
    <>{t('itemCount', { count: 1 })}</> // [!code highlight]
  );
}
```
