...
`useDict()` is used to access string translations from the [translation dictionary](/docs/react/reference/dictionaries/dictionaries).
...
<Callout>
  `useDict()` uses a [dictionary](/docs/react/reference/dictionaries/dictionaries) to store all content for translation.
...
| `options?`            | [`InlineTranslationOptions`](/docs/react/api/types/inlineTranslationOptions) | Translation options to customize the behavior of `d()`. |
