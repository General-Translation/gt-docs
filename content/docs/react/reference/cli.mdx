...
For a step by step guide, check out the [Deploying to Production](/docs/react/reference/cli-tool) guide.
...
Due to the nature of React being rendered on the client side, API Keys and similar credentials
...
Additionally, the CLI tool will automatically detect the framework you are using. If your project is
a Next.js app, it will automatically wrap `<GTProvider>` components at the top level `layout.tsx` of your app, as well as
