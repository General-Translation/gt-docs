---
title: <GTProvider>
description: Configuring `gt-react` with the provider
---
...
- [`useLocale()`](/docs/gt-react/helpers#uselocale): Retrieves the user's current language as an [ISO-639](https://en.wikipedia.org/wiki/List_of_ISO_639_language_codes) language code.
- [`useDefaultLocale()`](/docs/gt-react/helpers#usedefaultlocale): Retrieves the application's default language as an ISO-639 language code.
- [`useGT()`](/docs/gt-react/dictionaries): Used to initialize the `t()` function, which loads a translation from the dictionary if you are using the dictionary pattern.
