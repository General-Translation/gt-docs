...
description: gt-reactで5分であなたのReactアプリを国際化する
---
...
文字列の翻訳には、[`useGT()`](/docs/react/api/strings/useGT) を使用できます。
...
<Callout type="warn">
    **APIキーを保護してください！**
    ...
    Reactの環境変数は、ビルド時にアプリケーションにバンドルされ、クライアント側のコードで公開されることがよくあります。
...
<Accordions>
  <Accordion title="ブラウザの言語を変更したのに、アプリの言語が変わらない。">
    **ブラウザのクッキー**
    ...
    その後、希望する言語を使用していることを確認し、ページを更新してください。
  ...
  <Accordion title="新しい言語の読み込みに時間がかかるのはなぜですか？">
    **オンデマンド翻訳**
    ...
    開発中に言語を読み込むと、翻訳が表示されるまで数秒かかることがあります。
    ...
    [本番環境へのデプロイ](/docs/next/tutorials/quickdeploy) に関するガイドに従ってください。
  </Accordion>
...
[本番環境へのデプロイ](/docs/react/tutorials/quickdeploy)についてのガイドをご覧ください。

---

## 次のステップ
 * `<T>`コンポーネントやその他の利用可能なコンポーネントに関する詳細情報については、[React APIリファレンス](/docs/react/api/components/t)をご覧ください。
 * [Reactプラグイン](/docs/react/reference/local-management#include-locales)を使用して、アプリの承認済み言語のリストを作成します。
