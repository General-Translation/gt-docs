---
title: <Plural>
description: <Plural>コンポーネントのAPIリファレンス
---
 ...
 * あなたが `"en-US"` を使用している開発者である場合、2つのみを使用してください: `"singular"` と `"plural"`。
...
## 次のステップ
 * さらなる例については、ブランチングコンポーネントに関するリファレンスドキュメントを[こちら](/docs/react/reference/using-branching-components)でご覧ください。
 * より高度な使用法については、`<Plural>`を`<Currency>`, `<DateTime>`, `<Num>`, `<Var>`のような変数コンポーネントと組み合わせてください。変数コンポーネントについての詳細は[こちら](/docs/react/reference/using-variable-components)をご覧ください。

複数形化とブランチングについてさらに詳しく知りたい場合は、[Using Branching Components](/docs/react/reference/using-branching-components)を訪問してください。