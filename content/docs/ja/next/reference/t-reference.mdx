---
title: <T> リファレンス
description: <T> コンポーネントと gt-next ライブラリについての詳細な解説です。
---


## 概要

この記事では、`gt-next`ライブラリの[`<T>`コンポーネント](/docs/next/api/components/t)の使用方法について詳しく説明します。
...
[Variable Components](/docs/next/reference/using-variable-components)や[Branching Components](/docs/next/reference/using-branching-components)の使用方法をお探しの場合は、それぞれの記事を参照してください。

---

## `<T>` コンポーネントとは何ですか？

`<T>` コンポーネントは、`gt-next` ライブラリでテキストとコンテンツを翻訳するための主要な方法です。
...
これは、文字列の翻訳、事前のコンテンツ翻訳、プライバシーの懸念など、`<T>` コンポーネントに関連するが同様に重要なトピックにも触れます。

### `<GTProvider>` で `<T>` コンポーネントをラップする [#gtprovider]

`<T>` は静的なテキストと JSX 構造を翻訳するために使用されます。
クライアントサイドのコンポーネントの場合、`<T>` は必ず上位レベルで [`<GTProvider>`](/docs/next/api/components/gtprovider) でラップされている必要があります。
...
#### 例外: サーバーサイド `<T>` コンポーネント [#server-t]

サーバーサイドレンダリングの場合、`<GTProvider>` は `<T>` コンポーネントに対してオプションです。
...
多くの変数コンポーネントがあります: [`<Var>`](/docs/next/api/components/var), [`<Num>`](/docs/next/api/components/num), [`<Currency>`](/docs/next/api/components/currency) および [`<DateTime>`](/docs/next/api/components/datetime)。
...
[変数コンポーネントの使用に関するこの記事](/docs/next/reference/using-variable-components)で詳細を読むことができます。

### `<T>` コンポーネント内の分岐

分岐コンポーネントは、翻訳の条件に基づいて異なるコンテンツを表示するために使用できます。
これには、[`<Plural>`](/docs/next/api/components/plural) および [`<Branch>`](/docs/next/api/components/branch) コンポーネントが含まれます。
...
[分岐コンポーネント](/docs/next/reference/using-branching-components)の記事で詳細を読むことができます。

### 文字列の翻訳 [#translating-strings]

文字列だけを翻訳したい場合は、[`useGT()`](/docs/next/api/components/strings/useGT), [`getGT()`](/docs/next/api/components/strings/getGT), または [`tx()`](/docs/next/api/components/strings/tx) を使用できます。

[`useGT()`](/docs/next/api/components/strings/useGT) および [`getGT()`](/docs/next/api/components/strings/getGT) は、ビルド時に文字列を翻訳するために使用されます。
...
[`tx()`](/docs/next/api/components/strings/tx) は、実行時に文字列を翻訳するために使用されます。
...
詳細については、[文字列の翻訳に関するガイド](/docs/next/reference/translating-strings)を参照してください。
...
この問題を回避するには、[変数コンポーネント](/docs/next/reference/using-variable-components#privacy)を使用してプライベート情報を処理してください。
これにより、機密データが一般翻訳APIに送信されることはありません。
変数コンポーネントでラップされたすべてのコンテンツのローカライズはローカルで処理されます。
...
つまり、コンポーネント内に `<T>` に直接渡されていないコンテンツがある場合、それは翻訳されません。
...
`<T>` 内に直接あるコンテンツのみが翻訳されます。例えば、`<h1>` タグとその子孫です。
...
最初の本能としては、`<UntranslatedContent>` 内に追加の `<T>` コンポーネントを追加することかもしれませんが、これは推奨されません。
...
変数は少し複雑です。なぜなら、それらは値を変更する傾向があるからです。
動的な値を表示するためには[Variable Components](/docs/next/reference/using-variable-components)を使用し、条件付きロジックには[Branching Components](/docs/next/reference/using-branching-components)を使用することをお勧めします。

実行時に動的なコンテンツを翻訳したい場合は、[`tx()`](/docs/next/api/components/strings/tx) と [`<Tx>`](/docs/next/api/components/tx) をチェックしてください。
...
コンテンツの翻訳を最適化するために、条件付きコンテンツを[Branching Component](/docs/next/reference/using-branching-components)でラップすることが常に最善の方法です。
 ...
 * クライアントコンポーネントでの翻訳を有効にするには、アプリを [`<GTProvider>`](/docs/next/api/components/gtprovider) でラップしてください。
 * `<T>` はビルド時にコンテンツを翻訳します。実行時ではありません。開発中は利便性のためにオンデマンド翻訳が行われますが、本番環境ではそうではありません。

## 次のステップ
 * [可変コンポーネントの記事](/docs/next/reference/using-variable-components)と[分岐コンポーネントのガイド](/docs/next/references/using-branching-components)を探索してください。
 * アプリケーションでの[文字列の翻訳](/docs/next/reference/translating-strings)について学びましょう。
 * 実行時の翻訳については、[`tx()`](/docs/next/api/components/strings/tx)と[`<Tx>`](/docs/next/api/components/tx)をチェックしてください。
