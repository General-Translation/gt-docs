...
[ロケールコード](/docs/reference/supported-locales)の配列を渡すことで、サポートしたい言語を指定できます。
...
<Callout type="info">
  **ヒント:**
  ...
  これにより、コードベースを翻訳可能なJSXについてスキャンし、[`<T>`](/docs/next/api/components/t)タグを自動的に挿入します。
...
文字列の場合、翻訳には[`useGT()`](/docs/next/api/strings/useGT)または[`getGT()`](/docs/next/api/strings/getGT)を使用できます。
...
- `<T>`コンポーネントやその他の利用可能なコンポーネントに関する詳細情報は、[Next.js APIリファレンス](/docs/next/api/components/t)をご覧ください。
