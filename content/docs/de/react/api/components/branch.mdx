  ...
  return (
    <T id="example">
      <Branch branch={user.hairColor}
        ...
        blonde={<p>Ihr Haar ist hell.</p>}
      >
        <p>Unbehandelte Haarfarbe: <Var>{user.hairColor}</Var></p> // [!code highlight]
      </Branch>
    </T>
  );
}
 ...
 * Kombinieren Sie `<Branch>` mit anderen Komponenten, wie `<T>` für Übersetzungen und [variable Komponenten](/docs/react/reference/using-branching-components) für dynamische Inhalte, um reichhaltige und lokalisierte Schnittstellen zu erstellen.

## Nächste Schritte
 * Für eine fortgeschrittene Nutzung und Beispiele, siehe [Verwendung von verzweigenden Komponenten](/docs/react/reference/using-branching-components).
 * Um mehr über variable Komponenten wie `<Currency>`, `<DateTime>`, `<Num>`, und `<Var>` zu erfahren, siehe die Dokumentation [Verwendung von variablen Komponenten](/docs/react/reference/using-variable-components).