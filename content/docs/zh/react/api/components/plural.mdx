...
| `children` | 如果没有找到匹配的复数分支，则渲染的备用内容。 |
...
包含与 `n` 的复数形式对应的内容的 `JSX.Element`，或备用内容。
...
## 注意事项
 * `<Plural>` 组件用于处理复数形式。
 * 可用的复数分支（例如，one, other, few, many）取决于语言环境，并遵循[Unicode CLDR 复数规则](https://cldr.unicode.org/index/cldr-spec/plural-rules)。

## 下一步
 * 欲了解更多示例，请查看有关分支组件的参考文档[此处](/docs/react/reference/using-branching-components)。
 * 对于更高级的用法，将 `<Plural>` 与变量组件如 `<Currency>`、`<DateTime>`、`<Num>` 和 `<Var>` 结合使用。阅读更多关于变量组件的信息[此处](/docs/react/reference/using-variable-components)。

如需深入了解复数化和分支，请访问[使用分支组件](/docs/react/reference/using-branching-components)。