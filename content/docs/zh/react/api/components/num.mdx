...
`<Num>` 组件以用户的本地语言环境渲染格式化的数字字符串，并且可以通过格式选项进行自定义。
```jsx
<Num>{100}</Num>
// 输出: 100
```
...
| `locales`   | 指定格式化语言环境的可选语言环境。如果未提供，则使用默认用户的语言环境。阅读更多关于指定语言环境的信息 [这里](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Intl#locales_argument)。                                            |
