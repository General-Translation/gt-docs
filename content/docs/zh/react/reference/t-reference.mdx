---
title: <T> 参考
description: 这是对 <T> 组件和 gt-react 库的详细探讨。
---

## 概述

本文将深入探讨如何在 `gt-react` 库中使用 [`<T>` 组件](/docs/react/api/components/t)。
请根据需要随意跳转到本页的不同部分。
我们将涵盖以下内容：
...
如果您正在寻找如何使用 [变量组件](/docs/react/reference/using-variable-components) 或 [分支组件](/docs/react/reference/using-branching-components)，请参考各自的文章。

---

## `<T>` 组件是什么？

`<T>` 组件是 `gt-react` 库中翻译文本和内容的主要方式。
它允许内联翻译文本和 JSX 结构，为在应用程序中管理翻译提供了一种更直接的方式。
这通常对开发人员更方便，因为它允许对翻译内容进行更直接的控制。

`<T>` 组件支持静态文本、JSX 结构和上下文感知的翻译。
当没有可用的翻译时，它会优雅地回退到显示应用程序的默认语言环境中的内容。
...
`<T>` 用于翻译静态文本和 JSX 结构。
`<T>` **必须** 在更高层级被 [`<GTProvider>`](/docs/react/api/components/gtprovider) 包装。
我们的建议是在根级别将你的应用程序包装在 `<GTProvider>` 中以提供此上下文。
...
有许多变量组件：[`<Var>`](/docs/react/api/components/var)、[`<Num>`](/docs/react/api/components/num)、[`<Currency>`](/docs/react/api/components/currency) 和 [`<DateTime>`](/docs/react/api/components/datetime)。
其中一些将根据用户的区域设置自动重新格式化内容。其他的则只是将内容标记为“不要翻译”。
此外，这些组件不一定需要包装在 `<T>` 组件中即可重新格式化内容。

在[这篇关于变量组件的文章](/docs/react/reference/using-variable-components)中阅读更多信息。

### `<T>` 组件中的分支

分支组件可以用于根据翻译条件显示不同的内容。
这些包括 [`<Plural>`](/docs/react/api/components/plural) 和 [`<Branch>`](/docs/react/api/components/branch) 组件。
...
你可以根据使用的组件指定不同的选项。
在[分支组件](/docs/react/reference/using-branching-components)文章中阅读更多信息。

### 翻译字符串 [#translating-strings]

如果你只想翻译一个字符串，可以使用 [`useGT()`](/docs/react/api/components/strings/useGT)。
它用于在构建时翻译字符串。
...
```
有关更多信息，你可以参考[我们的字符串翻译指南](/docs/react/reference/translating-strings)。

就是这样！你已经成功地将你的应用程序翻译成法语、西班牙语和日语。

有关部署应用程序的更详细指南，请参考[部署](/docs/react/tutorials/quickdeploy)指南。
有关命令的更多信息，请参考[CLI 工具](/docs/cli/translate)参考指南。

### 向 `<T>` 组件添加 ids

一个方便的调试技巧是向你的 `<T>` 组件添加一个 `id` 属性。
这允许你跟踪在你的应用程序中使用了哪些翻译。
它还允许你使用[翻译编辑器](/signin)在不重新部署的情况下更改翻译。
...
在开发中，`<T>` 组件将按需翻译内容。这意味着当组件被渲染时，它将立即执行翻译。我们这样做是为了方便，使得与其他语言的开发更容易。
...
<Callout>
  **提示：**
  如果您想在开发中模拟生产行为，只需在您的开发构建中使用生产 API 密钥。
</Callout>
...
为了解决这个问题，请使用[变量组件](/docs/react/reference/using-variable-components#privacy)来处理私人信息。这确保没有敏感数据被发送到通用翻译 API 进行翻译。所有包裹在任何变量组件中的内容的本地化都在本地处理。
...
我们建议使用[变量组件](/docs/react/reference/using-variable-components)来显示动态值，并使用[分支组件](/docs/react/reference/using-branching-components)来处理条件逻辑。

### 嵌套 `<T>` 组件 [#nested-t]

不允许嵌套 `<T>` 组件。
由于 React 的渲染系统，这可能会导致按需翻译时出现意外行为和性能问题。
...
最佳实践是将条件内容包装在[分支组件](/docs/react/reference/using-branching-components)中，以优化内容的翻译。
...
## 注意事项
 * `<T>` 组件用于翻译任意 JSX 内容。
 * 使用 [`<GTProvider>`](/docs/react/api/components/gtprovider) 包裹您的应用以启用翻译。
 * `<T>` 在构建时翻译内容，而不是在运行时。开发过程中会为了方便进行按需翻译，但在生产环境中并非如此。

## 下一步
 * 探索[变量组件文章](/docs/react/reference/using-variable-components)和[分支组件指南](/docs/react/references/using-branching-components)。
 * 了解在应用程序中[翻译字符串](/docs/react/reference/translating-strings)。
