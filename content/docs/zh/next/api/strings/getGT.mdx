---
title: getGT()
description: getGT() 字符串翻译函数的 API 参考
---

## 概述

`getGT()` 函数是一个用于在构建时翻译字符串的服务器端函数。
...
这确保了所有语言环境的快速加载时间，但它只能翻译在构建时已知的内容。
...
## 注意事项
 * `getGT()` 函数是一个服务器端函数，用于翻译字符串。
 * 使用 `getGT()` 的翻译字符串在运行时之前发生，在构建过程中进行（除非在开发中）。

## 下一步

 * 查看 [`useGT()`](/docs/next/api/strings/useGT) 以获取构建时客户端字符串翻译。
