...
`<DateTime>` 组件渲染格式化的日期或时间字符串，支持格式选项和区域设置等自定义。
...
| `locales`  | 可选的语言环境，用于指定格式化的语言环境。如果未提供，则使用用户的语言环境。阅读更多关于指定语言环境的信息 [这里](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Intl#locales_argument)。|
...
## 注意事项
 * `<DateTime>` 组件是一个可变组件，可以用于格式化日期和时间值。
...
## 下一步
 * 有关 `<DateTime>` 组件和其他变量组件如 `<Currency>`、`<Num>` 和 `<Var>` 的更多详细信息和使用示例，请参阅 [使用变量组件](/docs/next/reference/using-variable-components) 文档。