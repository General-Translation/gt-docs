...
使用 [`<Var>` 组件](/docs/next/api/components/var) 来指定不应该被翻译的 JSX 内容。
...
<Steps>
  <Step>
    导航到[控制面板](https://generaltranslation.com/en-US/signin)。
    在侧边栏中进入**开发者密钥**页面。
  </Step>
...
- 了解如何通过 [i18n 路由](/docs/next/reference/i18n-routing)（为每个语言环境添加路由，例如 `example.com/en`、`example.com/fr`）来提高 SEO。
- 镜像您的应用以支持[从右到左的语言](/docs/next/reference/rtl)，如阿拉伯语和希伯来语。
