...
Additionally, the CLI tool can automatically scan your codebase for content to be translated.

```bash title="shell" copy
npx gt-cli scan
```

The scan will wrap `<T>` components around the top level of JSX elements which contain translatable content.
...
a Next.js app, it will also automatically wrap `<GTProvider>` components at the top level `layout.tsx` of your app.
