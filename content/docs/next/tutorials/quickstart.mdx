...
<Steps>
    ...
    <Step>
        Run the CLI tool to scan your codebase
    </Step>
    <Step>
...
Install the [gt-next](https://www.npmjs.com/package/gt-next) and [gt-react-cli](https://www.npmjs.com/package/gt-react-cli) libraries.
...
<Tabs items={["npm", "yarn", "bun", "pnpm"]}>
  ...
  <Tab value="yarn">
  ```bash 
  yarn add gt-next gt-react-cli
  ```
  </Tab>
  <Tab value="bun">
  ```bash 
  bun add gt-next gt-react-cli
  ```
  </Tab>
  <Tab value="pnpm">
  ```bash 
  pnpm add gt-next gt-react-cli
  ```
  </Tab>
</Tabs>
...
Run the [gt-react-cli](https://www.npmjs.com/package/gt-react-cli) tool to scan your codebase for translatable content and automatically wrap it in `<T>` components.

```bash title="shell" copy
npx gt-react-cli scan
```

See the [CLI tool documentation](/docs/reference/cli#scanning) for more information.
...
### 3. Modify the root layout

Add the `lang` attribute to your root layout with `await getLocale()`.

<Callout>
    The CLI tool should automatically add the [`<GTProvider>`](/docs/next/api/components/gtprovider) component to your root layout.
    If it did not, you should add it manually. This is necessary to enable translations for client-side `<T>` components.
</Callout>
...
<Callout type="info">
    **Security**
    ...
    you must pre-authorize your content before it can be translated.
