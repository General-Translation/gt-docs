...
Let's say that you have added the [`gtx-cli translate`](/docs/next-cli/translate) command to your CD process.
...
<Callout>
  **Note:**
  If you are not using GT infrastructure, you will need to write a custom [`loadTranslation()`](/docs/next/api/config/load-translation) method to load translations from your preferred infrastructure.
</Callout>
...
<Steps>
  ...
  <Step>
    In your [translate](/docs/next-cli/translate) command, add the following flags to use local translations:
      * `--no-publish`: Disable translations from being published to the GT CDN.
      * `--translations-dir ./public/_gt`: Save translations to the `./public/_gt` directory.
    
    ```bash
    npx gtx-cli translate --no-publish --translations-dir ./public/_gt
    ```
  </Step>
</Steps>
...
We currently do not yet support bringing your own translations.
Adding this functionality is one of our top priorities, and it will be available very soon.

Until then, we recommend using the generated translations.
You can do this by marking the file with your preferred language as your dictionary file in `gt.config.json`.

For example, say that `./messages/en.json` is your base language file:
```json
{
  "projectId": "your-project-id",
  "dictionary": "./messages/en.json",
  "locales": ["en", "fr", "de"],
  ...
}
```

And add your load translation function:
```js title="src/loadTranslation.js" copy
export default async function loadTranslation(locale) {
  const t = await import(`./_gt/${locale}.json`);
  return t.default;
...
When you run the translate command, it will generate translations in the `./public/_gt` directory.
```bash
npx gtx-cli translate --no-publish --translations-dir ./public/_gt
```
...
```jsx title="pages/index.js" copy
import { useDict } from 'gt-next/client';
import { getDict } from 'gt-next/server';

export default function Home() {
  const { t } = useDict(); // client-side // [!code highlight]
  const { t } = getDict(); // server-side // [!code highlight]
  return (
    <div>
      <h1>{t('identifier')}</h1> // [!code highlight]
    </div>
  );
}
```
 ...
 * You can customize [`loadTranslation()`](/docs/next/api/config/load-translation) to also load translations from other sources, such as your database or your own CDN.

## Next steps
 * See [`loadTranslation()`](/docs/next/api/config/load-translation) for more information on writing a custom translation loader.
