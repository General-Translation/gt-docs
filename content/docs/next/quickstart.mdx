...
**In production, we recommend using the `gt-react-cli` CLI tool to request translations, rather than translating everything on demand.**
...
Learn more about `gt-react-cli` [here](/docs/next/cli).
