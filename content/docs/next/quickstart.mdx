...
- [`<T>`](/next/t), with which you mark the content you want to translate
- [`<GTProvider>`](/next/gt-provider), which provides translations and language information from the server to client-side components using React's context API
...
- [Plural forms](/next/plurals) with the `<Plural>` component
- [Formatted variables](/next/variables) like numbers, currencies, and dates
...
Learn more about `gt-update` [here](/docs/next/cli).

## Congratulations! Your app is now multilingual! 🥳

### Next steps

- Learn more about the [`<T>`](/docs/next) component, or dictionaries with [`getGT()`](/docs/next/dictionaries)
