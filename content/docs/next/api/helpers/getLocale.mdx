...
<Callout>
    `getLocale()` is a server-side method and cannot be used in client components. For client-side locale management, see [`useLocale()`](/docs/next/api//helpers/useLocale).
</Callout>
