...
If no default locale is specified in [`initGT()`](/docs/next/api/config/initGT) it defaults to `'en-US'`.
...
## Notes
 * The `getDefaultLocale()` returns the default locale set in [`initGT()`](/docs/next/api/config/initGT).
