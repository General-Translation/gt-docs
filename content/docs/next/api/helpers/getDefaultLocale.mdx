...
See [`initGT()`](/docs/next/api/config/withGTConfig) for configuration.
If no default locale is specified in [`initGT()`](/docs/next/api/config/withGTConfig) it defaults to `'en-US'`.
...
## Notes
 * The `getDefaultLocale()` returns the default locale set in [`initGT()`](/docs/next/api/config/withGTConfig).
