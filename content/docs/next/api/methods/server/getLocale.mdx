...
<Callout>
    `getLocale()` is a server-side method and cannot be used in client components. For client-side locale management, see [`useLocale()`](/docs/next/api/methods/client/useLocale).
</Callout>
 ...
 * It is specifically designed for server-side use. See [`useLocale()`](/docs/next/api/methods/client/useLocale) for client-side components.
 ...
 * For managing fallback locales, see [`getDefaultLocale()`](/docs/next/api/methods/server/getDefaultLocale).