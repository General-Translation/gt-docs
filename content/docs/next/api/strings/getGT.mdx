...
This ensures fast load times for all locales.
...
Remember to add a development API key to your environment to enable this behavior.
...
see [`<Tx>`](/docs/next/api/components/tx) or [`tx()`](/docs/next/api/strings/tx) for more information.
...
## Next steps
 * See [`useGT()`](/docs/next/api/dictionary/useGT) for client-side string translations at buildtime.
