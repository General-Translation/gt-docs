...
- For JSX or client-side translations, consider using [`useElement()`](/docs/next/api/dictionary/use-element) or [`getGT()`](/docs/next/api/dictionary/getGT).

<Callout>
    The `tx()` function is server-side only and cannot be used in client-side components. For client-side translations, see [`useGT()`](/docs/next/api/dictionary/useGT).
</Callout>
