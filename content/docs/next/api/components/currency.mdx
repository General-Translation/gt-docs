...
### Dictionary Examples
The `<Currency>` component can also be used in a dictionary entry.
All of the above formatting can also be applied when using a dictionary.
...
const dictionary = {
    "itemPrice": <>
        The price is <Currency>{10}</Currency>. // [!code highlight]
    </>,
};
...
const dictionary = {
    "itemPrice": <>The price is <Currency name='price'/>.</>, // [!code highlight]
};
export default dictionary;
```
...
export default function PriceDisplay(item) {
  ...
  return (
    <div>
      { t('itemPrice', { price: item.price }) } // [!code highlight]
    </div>
  );
}
```
