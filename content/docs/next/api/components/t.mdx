 ...
 * For translating strings, look into [`tx()`](/docs/next/api/methods/server/tx) or [`getGT()`](/docs/next/api/methods/server/getGT) for server-side translations and [`useGT()`](/docs/next/api/methods/client/useGT) for client-side translations.
