...
const dictionary = {
  hairColor: <>
    <Branch branch="black" // [!code highlight]
...
In order to pass values, you must specify a name for the `<Branch>` (or other) component(s) and reference it when calling the `t()` function.
