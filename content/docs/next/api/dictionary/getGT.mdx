...
`getGT()` is used to get translations from the [translation dictionary](/docs/next/reference/dictionaries/dictionaries) for server-side components.
...
For client-side translations, see [`useGT()`](/docs/next/api/dictionary/useGT).
...
export default function UserDetails() {
  const t = await getGT('prefix1.prefix2'); // [!code highlight]
  return (
    <div>
...
## Next Steps
 * See [`useGT()`](/docs/next/api/dictionary/useGT) for the client-side equivalent of `getGT()`.
