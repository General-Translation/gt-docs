...
`useDict()` is used to access string translations from the [translation dictionary](/docs/next/reference/dictionaries/dictionaries) for client-side components.
...
| `options?`            | [`InlineTranslationOptions`](/docs/next/api/types/inlineTranslationOptions) | Translation options to customize the behavior of `d()`. |
