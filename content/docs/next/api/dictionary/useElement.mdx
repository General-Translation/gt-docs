...
The `useElement()` method allows you to access jsx content from your [translation dictionary](/docs/next/reference/dictionaries/dictionaries).
...
`useElement()` supports:
  * Jsx translation only (no string support, use [`getGT()`](/docs/next/api/dictionary/getGT), [`useGT()`](/docs/next/api/dictionary/useGT), or [`tx()`](/docs/next/api/strings/tx) for string translations).
...
<Callout>
  `getGT()`, `useGT()`, and `useElement()` use a [dictionary](/docs/next/reference/dictionaries/dictionaries) to store all content for translation.
...
Despite its name, `useElement()` is not always a hook.
...
`useElement()` excludes strings as this allows for identical syntax in both client and server side components.
For accessing both strings and jsx content, use [`getGT()`](/docs/next/api/dictionary/getGT) or [`useGT()`](/docs/next/api/dictionary/useGT).
...
Just use a [variable component](/docs/next/reference/dictionaries/variable-components).
...
export default dictionary;
```
Because we added the value `'prefix1.prefix2'` to the `useElement()` method, all of the keys are prefixed with `prefix1.prefix2`:
 ...
 * `useElement()` is "platform agnostic", meaning it can be used in both client-side and server-side components.
 * For string-based translations, consider using [`useGT()`](/docs/next/api/dictionary/useGT), [`getGT()`](/docs/next/api/dictionary/getGT), or [`tx()`](/docs/next/api/strings/tx).

## Next Steps
 * Read more about the [dictionary design pattern](/docs/next/reference/dictionaries/dictionaries).
 * Combine `useElement()` with [Variable Components](/docs/next/reference/dictionaries/variable-components) and [Branching Components](/docs/next/reference/dictionaries/branching-components) for advanced translation scenarios.