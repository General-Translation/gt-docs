...
`getDict()` is used to get translations from the [translation dictionary](/docs/next/reference/dictionaries/dictionaries) for server-side components.
...
A promise that resolves to the `d()` translation function with the following signature:
