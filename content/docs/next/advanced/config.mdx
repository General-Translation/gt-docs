...
- In the [`gt-react-cli`](/cli) CLI tool so you can push dictionaries from the command line(`GT_API_KEY`)
...
<Callout>
    If you are using a [`dictionary.js`](/docs/next/dictionaries) file, use the `withGT()` plugin returned by `initGT()` to configure [Webpack](https://nextjs.org/docs/app/api-reference/next-config-js/webpack) to bundle your `dictionary.js` file into your app.
</Callout>
...
#### `projectID`
- **Type**: `string`
- **Description**: Project ID, which can be included here or as an [environment variable](/docs/next/config#set-environment-variables).
