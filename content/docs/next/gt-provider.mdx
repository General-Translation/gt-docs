...
- [`getLocale()`](/docs/next/helpers#getlocale): Retrieves the user's current language as an [ISO-639](https://en.wikipedia.org/wiki/List_of_ISO_639_language_codes) language code.
- [`getDefaultLocale()`](/docs/next/helpers#getdefaultlocale): Retrieves the application's default language as an ISO-639 language code.
- [`useElement()`](/docs/next/dictionaries): Used to initialize the `t()` function, which loads a translation from the dictionary, if you are using a dictionary pattern.
