...
export default function MyComponent() {
  ...
  const d = useDict(); // on the client side, you use the useGT hook
...
Read more about adding variables to your dictionary in the reference docs for [`useGT()`](/docs/next/api/dictionary/useGT) and [`getGT()`](/docs/next/api/dictionary/getGT).


### Prefixes

Additionally, with `useGT()` and `getGT()` you can pass in a prefix to the function to specify a shared path in the dictionary.
...
export default async function MyComponent() {
  ...
  const d = await getGT('greetings.common'); // [!code highlight]
...
For more information on the command, please refer to the [CLI Tool](/docs/next-cli/translate) reference guide.
 ...
 * Dictionary translations occur at build time, so you have to add the [`translate`](/docs/next-cli/translate) command to your build process.
