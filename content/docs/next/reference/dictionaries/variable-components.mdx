...
description: Inserting variables into dictionary entries with variable components
---
...
[`<Currency>`](/docs/next/api/components/currency), and [`<DateTime>`](/docs/next/api/components/datetime) within dictionaries.
...
## Why Use Variable Components with Dictionaries? [#motive]
...
This is done through the options parameter in methods like [`useGT()`](/docs/next/api/dictionary/useGT) or [`getGT()`](/docs/next/api/dictionary/getGT).
 ...
 * Use the options parameter in [`useGT()`](/docs/next/api/dictionary/useGT) or [`getGT()`](/docs/next/api/dictionary/getGT) to pass dynamic data when accessing dictionary entries.

## Next Steps
 * Explore how to use [Branching Components](/docs/next/reference/dictionaries/branching-components) for conditional logic in translations.
 * Learn more about [Template Dictionary Design Pattern](/docs/next/reference/dictionaries/dictionaries).
