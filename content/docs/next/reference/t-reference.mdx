...
Right now, the best way to translate a string is to use the [Dictionary Design Pattern](/docs/next/reference/dictionaries/dictionaries).
...
Then you can call it using the [`useGT()`](/docs/next/api/dictionary/useGT) hook or [`getGT()`](/docs/next/api/dictionary/getGT) method for client and server-side components, respectively.
...
Translating high frequency languages ahead of time is a common practice to improve load times.
We have built a simple CLI tool that allows you to specify which languages you would like to translate.
It can easily integrate with your CI/CD pipeline to ensure that translations are always up to date.

```bash
npx gt-next-cli translate --languages fr es ja
```

That's it! You have successfully translated your application into French, Spanish, and Japanese.

For more information, please refer to the [CLI Tool](/docs/next-cli/translate) reference guide.
...
<Callout>
    **Note:** The `id` prop is optional, but may be useful for debugging and observability.
    Additionally, the translation editor feature on the platform will be easier to use if you provide a period separated `id`.
</Callout>
...
3. If you want to render a string, please use the [Dictionary Design Pattern](/docs/next/reference/dictionaries/dictionaries).
