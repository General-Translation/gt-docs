...
<Steps>
    <Step>
        Navega al [Panel de Control](https://generaltranslation.com/dashboard).
    ...
    <Step>
        Añade el ID del Proyecto y la Clave API a tus variables de entorno.
        Dependiendo de tu framework de React, las variables de entorno pueden verse ligeramente diferentes.
        Estas instrucciones son para **ENTORNOS DE DESARROLLO ÚNICAMENTE**.
        <Tabs items={["Vite", "Gatsby", "RedwoodJS", "create-react-app", "Other"]}>
            ...
            En entornos de desarrollo, tanto `GT_API_KEY` como `GT_PROJECT_ID` necesitan ser exportados al cliente.
...
<Callout type="warn">
    **¡Protege Tus Claves API!**
    ...
    Las variables de entorno de React a menudo se empaquetan en tu aplicación durante el tiempo de compilación y son visibles públicamente en el código del lado del cliente.
...
<Accordions>
  ...
  <Accordion title="¿Por qué los nuevos idiomas tardan mucho en cargarse?">
    **Traducción bajo demanda**
    ...
    Esto **solo ocurre en desarrollo** para que puedas prototipar fácilmente tu sitio web en diferentes idiomas.
