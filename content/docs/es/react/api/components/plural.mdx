...
| `locales`   | Locales opcionales para especificar el formato del locale. Si no se proporciona, se utiliza el locale predeterminado del usuario. Lee más sobre cómo especificar locales [aquí](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Intl#locales_argument).                                            |
...
## Próximos pasos
 * Para más ejemplos, consulta el documento de referencia sobre componentes de ramificación [aquí](/docs/react/reference/using-branching-components).
 * Para un uso más avanzado, combina `<Plural>` con componentes variables como `<Currency>`, `<DateTime>`, `<Num>`, y `<Var>`. Lee más sobre componentes variables [aquí](/docs/react/reference/using-variable-components).

Para profundizar en la pluralización y ramificación, visita [Uso de Componentes de Ramificación](/docs/react/reference/using-branching-components).