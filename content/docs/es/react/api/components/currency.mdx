...
| `locales`   | Locales opcionales para especificar el locale de formato. Si no se proporciona, se usa el locale predeterminado del usuario. Lee más sobre cómo especificar locales [aquí](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Intl#locales_argument).                                            |
...
Aquí estamos mostrando el precio en GBP que especifica exactamente los lugares decimales y utiliza el símbolo estrecho para la moneda (es decir, "$100" en lugar de "US$100").
