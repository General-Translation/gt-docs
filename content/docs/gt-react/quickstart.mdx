...
See a full list of `<GTProvider>` props [here](/gt-react/gt-provider#config).
...
- [Plural forms](/gt-react/plurals) with the `<Plural>` component
- [Formatted variables](/gt-react/variables) like numbers, currencies, and dates
- [Context](/gt-react#context) for the AI model to use while translating
...
Learn more about `gt-update` [here](/gt-react/cli).

## Congratulations! Your app is now multilingual! 🥳

## Next steps

- Learn more about the [`<T>`](/gt-react) component, or dictionaries with [`useGT()`](/gt-react/dictionaries)
