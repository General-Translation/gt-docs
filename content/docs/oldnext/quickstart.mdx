...
To translate your app, wrap the content you want to translate in a [`<T>` component](/docs/oldnext/t).
...
<Callout>
    ...
    For dynamic content, use the [`<Var>` component](/docs/oldnext/variables).
    {/* TODO:
        The text and structure of the tags inside your `<T>` component should be static.
        Translation of dynamic content is covered in in [this guide](/docs/oldnext/dynamic-content).
     */}
</Callout>
...
To add translations to client `<T>` components, wrap your app in the [`<GTProvider>` component](/docs/oldnext/gt-provider).
...
For more information, see the [CLI tool documentation](/docs/oldnext/cli).
<Callout type="warn">
    **For Production Only!**
...
- Learn more about the [`<T>`](/docs/next) component, or dictionaries with [`useElement()`](/docs/oldnext/dictionaries)
- Learn to improve SEO with [i18n routing](/docs/oldnext/routing) (adding routes for each locale, e.g. `example.com/en`, `example.com/fr`)
- Mirror your app to support [right-to-left languages](/docs/oldnext/advanced/rtl) such as Arabic and Hebrew.
- Create a list of approved languages for your app with the [Next.js plugin](/docs/oldnext/config#install-the-plugin).
