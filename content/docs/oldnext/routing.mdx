...
You should include `defaultLocale` and `locales` in both `middleware.js` and [`next.config.js`](/docs/oldnext/config). The reason you have to include them separately is that on certain hosts (e.g. Vercel), middleware runs in a different environment, meaning `createNextMiddleware()` does not have access to your `initGT()` configuration.
