...
The `useGT` hook provides access to the `t` translation function from the [`<GTProvider>` context](/docs/next2/api/components/gtprovider).
This function enables you to retrieve the translated version of an entry from your [template dictionary](/docs/next2/reference/template-dictionaries/template-dictionaries).
...
The `useGT` hook retrieves the `t` function, which can translate any key from your [template dictionary](/docs/next2/reference/template-dictionaries/template-dictionaries).
...
Dynamic values can also be used alongside the `t` function to insert variables into the translated content via the [`<Var>` component](/docs/next2/api/components/var).
...
## Notes
 * The `useGT` hook can only be used within a component wrapped by a [`<GTProvider>` component](/docs/next2/api/components/gtprovider). Ensure your application is properly set up with a provider.
 ...
 * Explore [i18n configuration](/docs/next2/reference/i18n-configuration) to manage your dictionaries effectively.
 * Combine `useGT` with context hooks like [`useLocale`](/docs/next2/api/methods/client/useLocale) for more dynamic translation workflows.
