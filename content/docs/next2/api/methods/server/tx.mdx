...
- For JSX or client-side translations, consider using [`useElement()`](/docs/next2/api/methods/use-element) or [`getGT()`](/docs/next2/api/methods/server/getGT).

<Callout>
    The `tx()` function is server-side only and cannot be used in client-side components. For client-side translations, see [`useGT()`](/docs/next2/api/methods/client/useGT).
</Callout>
...
## Next Steps
 * Learn more about using variables in translations with [Variable Components](/docs/next2/reference/using-variable-components).
 * For JSX translations, see [`useElement()`](/docs/next2/api/methods/useElement), [`useGT()`](/docs/next2/api/methods/client/useGT) or [`getGT()`](/docs/next2/api/methods/server/getGT).
 * Explore locale management with [i18n Configuration](/docs/next2/references/i18n-configuration).