...
## Overview
[`<T>`](/docs/next2/components/t) is the translation tag in `gt-next`.
Children of `<T>` will be translated into your [configured](/docs/next2/reference/i18n-configuration) languages.

<Callout>
    To create translations, run the [CLI tool](/docs/next/cli) before you build your app.
</Callout>

<Callout type="warn">
    If it is being used in a client component, `<T>` must be able to access the context from a [`<GTProvider>`](/docs/next2/api/components/gt-provider).
</Callout>
...
If a translation is unavailable, `<T>` will default to displaying the content in your default locale ([`defaultLocale`](/docs/next2/reference/i18n-configuration#props)), usually English.
...
Dynamic variables can be injected into translations using components like [`<Var>`](/docs/next2/api/components/var).
 ...
 * Combine `<T>` with [Variable Components](/docs/next2/reference/using-variable-components) and [Branching Components](/docs/next2/reference/using-branching-components) for more complex translation scenarios.

## Next Steps
 * Learn about setting up translations alternate design patterns such as [Template Dictionaries](/docs/next2/reference/template-dictionaries/template-dictionaries).
 * Explore other localization techniques like [i18n Configuration](/docs/next2/reference/i18n-configuration) and [i18n Routing](/docs/next2/reference/i18n-routing).