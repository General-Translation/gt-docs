...
- [`<Var>`](/docs/next2/api/components/var): For rendering variables.
- [`<Num>`](/docs/next2/api/components/num): For formatting numbers with localization.
- [`<Currency>`](/docs/next2/api/components/currency): For formatting currency values with localization.
- [`<DateTime>`](/docs/next2/api/components/datetime): For formatting dates and times.
...
- [`<Var>`](/docs/next2/api/components/var): Displays general variables, such as user names or identifiers.
- [`<Num>`](/docs/next2/api/components/num): Formats numerical values according to locale-specific rules.
- [`<Currency>`](/docs/next2/api/components/currency): Formats currency values with symbols and localization rules.
- [`<DateTime>`](/docs/next2/api/components/datetime): Formats dates and times using locale-specific conventions.
...
For example:
* [`<Num>`](/docs/next2/api/components/Num) can display numbers with localized decimal separators.
* [`<Currency>`](/docs/next2/api/components/Currency) can format values with the correct currency symbol and positioning.
* [`<DateTime>`](/docs/next2/api/components/DateTime)can format dates and times according to locale-specific rules.
...
* [`<Var>`](/docs/next2/api/components/var): Private information that does not change format based on locale: e.g., user names, account numbers.
* [`<Num>`](/docs/next2/api/components/Num): Private numerical values that should be formatted according to locale: e.g., order quantities, age, distance.
* [`<Currency>`](/docs/next2/api/components/Currency): Private currency values that should be formatted according to locale: e.g., transaction amounts, account balances.
* [`<DateTime>`](/docs/next2/api/components/DateTime): Private date and time values that should be formatted according to locale: e.g., account creation dates, order timestamps.

It should be noted that the same is not true for [Branching Components](/docs/next2/reference/using-branching-components)
and [`<T>` components](/docs/next2/reference/t-reference), which do send data to the General Translation APIs for translation.
 ...
 * Components like [`<Num>`](/docs/next2/api/components/num), [`<Currency>`](/docs/next2/api/components/currency), and [`<DateTime>`](/docs/next2/api/components/datetime) provide localization features to ensure accurate formatting
 whereas the [`<Var>`](/docs/next2/api/components/var) component is used for general variables that do not require reformatting.
