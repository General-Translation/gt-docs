...
If you really need to put `<GTProvider>` on the client, import `<GTProvider>` from `'gt-next/client'` (not recommended in server-first apps), and follow the [`gt-react`](/gt-react/quickstart#add-gtprovider) documentation.