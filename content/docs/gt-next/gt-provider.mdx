...
- [`getGT()`](/gt-next/dictionaries): Used to initialize the `t()` function, which loads a translation from the dictionary, if you are using a dictionary pattern.
...
It's better to import `<GTProvider>` in a file not marked `'use client'`, so that it can fetch translations on the server-side. 
If you really need to put `<GTProvider>` on the client, import `<GTProvider>` from `'gt-next/client'` (not recommended in server-first apps), and follow the [`gt-react`](/gt-react/gt-provider) documentation.