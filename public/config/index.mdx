...
The `gt-react` library is initialized using the `initGT()` function. This function accepts an object with various configuration options.
...
<Callout>Besides `apiKey` and `projectID`, which should be set in the environment, all parameters are optional. This means `gt-react` functions can be initialized by calling `initGT()` with no arguments.</Callout>
